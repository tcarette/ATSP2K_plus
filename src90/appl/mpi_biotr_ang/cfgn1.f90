...
!     :                 QNOC,QNELCSH,QNOCORB,QJ1,QIAJCLD,QLJCLSD)
      ...
      if (myid == 0) print*, 'Closed shells ', nclosd
!
!     Now deal with the non-orthogonality
!
      IF (NWF > 1) THEN 
         if (myid == 0) WRITE (6, *) &
            ' alloc, qiorth: nwf*(nwf-1)/2 = ', NWF*(NWF - 1)/2 
...
!
   ...
   79 CONTINUE 
      ...
      IF (IBUFF(1)/=IASTER .AND. IBUFF(1)/=IBLNK) THEN 
         ...
         IORTH(IJ) = 0 
         WRITE (IWRITE, '(1X,A3,A,A3)') IBUFF(1), &
              ' is orthogonal to ', IBUFF(2) 
         GO TO 79 
      ENDIF 
   90 CONTINUE 
      RETURN  
