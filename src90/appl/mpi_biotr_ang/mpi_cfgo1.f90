...
!GG    6 FORMAT(15(1X,I1,A1,I1))
   ...
   25 FORMAT(/14H CONFIGURATION,I5,' :'                                 &
     & ,8(1X,A3,1H(,I2,1H)))
!GG   26 FORMAT(4X,17H COUPLING SCHEME:,8(1X,4X,I1,A1,I1))
...
!      call alloc(qljclsd,nwcd,4)
      allocate(iajcmp(nwfd),STAT=ierr)
      allocate(ljcomp(nwfd),STAT=ierr)
      allocate(njcomp(nwfd),STAT=ierr)
      allocate(noccsh(ncfg),STAT=ierr)
      allocate(nelcsh(8,ncfg),STAT=ierr)
      allocate(nocorb(8,ncfg),STAT=ierr)
      allocate(J1QNRD(15,ncfg),STAT=ierr)
      allocate(iajcld(nwfd),STAT=ierr)
      allocate(ljclsd(nwfd),STAT=ierr)
...
30    CONTINUE
      if (myid ==0) WRITE(IWRITE,22) NCFG
      if (myid ==0) WRITE(IWRITE,23) MAXORB,(IAJCMP(I),I=1,MAXORB)
      DO 60 I=1,MAXORB
...
!
 ...
 70   CONTINUE
      if (myid == 0) WRITE(IWRITE,28) NCLOSD,(IAJCLD(I),I=1,NCLOSD)
!
...
!     WRITE(IWRITE,25) I,(NOCORB(J,I),NELCSH(J,I),J=1,N)
   ...
   61 IF(NOCORB(J,I).EQ.IAJCMP(JJ)) NOCORB(J,I)=JJ
      M=2*N-1
      N1=N+1
!GG      READ(IREAD,6)    (J3QN(J),JCQN(J),J1QN(J),J=1,M)
...
!      J3QN(J) = NUMVAL(J3QNG(J))
!      J3QN(J) = CHAR(J3QNG(J)+ICHAR('0'))
