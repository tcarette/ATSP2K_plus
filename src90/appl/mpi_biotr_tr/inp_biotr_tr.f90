...
!-----------------------------------------------
!
      WRITE (ISCW, *) ' Type of realtivistic transition ? ', &
                      '(E1, E2, M1, M2, .. or *) '
...
!-----------------------------------------------
      ...
      integer, DIMENSION(2), INTENT(IN) :: NCF
...
!###
! start calculation of transitons
!###
...
!-----------------------------------------------
      ...
      inquire(FILE=file_name(1:L), exist=Y)
      IDX = index(file_name,' ');

      if (.not.Y) then
            write (iscw,'(A,A)') file_name(1:L), &
