...
! Spin-spin operator
      ...
      IF (IW1(KS1+1,KS2+8) == 0) THEN 
         ...
         DO K2KS3 = IPS1, IGS1, 2 
            ...
            IF (ISES1 /= 0) CALL SIXJ (2*KS1, 2*KS2, 2*K, K2KS3, 1, 1, 0, SN1) 
            IF (ISES2 /= 0) CALL SIXJ (2*KS2, 2*KS1, 2*K, K2KS3, 1, 1, 0, SN2) 
            DO I1 = IPS2, 2 
               KKS1 = I1 - 1 
      ...
      IF (IWAA(2,1,KL1+1,KL2+1) == 0) THEN 
         ...
         DO I3 = IP1, IG1 
            ...
            IF (ISES1L /= 0) CALL SIXJ (2*KL1, 2*KL2, 2*K, 2*KKL3, 2*ID4(3), 2*&
               ID1(3), 0, S3) 
            IF (ISES2L /= 0) CALL SIXJ (2*KL2, 2*KL1, 2*K, 2*KKL3, 2*ID4(3), 2*&
               ID1(3), 0, S4) 
            DO I4 = IP2, IG2 
               KKL1 = I4 - 1 
               ...
               IF (ISES1L*ISES3L /= 0) THEN 
                  CALL SIXJ (2*KL2, 2*ID3(3), 2*ID2(3), 2*KKL1, 2*ID1(3), 2*&
                     KKL3, 0, SLA1) 
                  SLA1 = SLA1*DSQRT(DBLE((2*KKL1 + 1)*(2*KKL3 + 1))) 
                  CA1 = CA1 + S3*SLA1*RECL1*RECL2*RECL3 
               ENDIF 
               IF (ISES2L*ISES4L == 0) CYCLE  
               CALL SIXJ (2*KL1, 2*ID3(3), 2*ID2(3), 2*KKL1, 2*ID1(3), 2*KKL3, &
                  0, SLB1) 
         ...
         END DO 
         IF (IREZ == 1) THEN 
            IF (MOD(ID1(3)+ID2(3)+ID3(3)+ID4(3)+KL1+KL2,2) /= 0) CA1 = -CA1 
            IF (MOD(ID1(3)+ID2(3)+ID3(3)+ID4(3),2) /= 0) CB1 = -CB1 
         ELSE 
            IF (MOD(KL1 + KL2,2) /= 0) CB1 = -CB1 
         ENDIF 
