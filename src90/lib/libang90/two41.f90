...
! Spin-spin operator
      ...
      DO I1 = IPS1, 2 
         ...
         DO I2 = IPS2, 2 
            ...
            DO I3 = IP1, IG1 
               KKL2 = I3 - 1 
               CALL A1A2W3LSP (IC, IA, IB, IC, KKL2, KKS2, (-HALF), (-HALF), &
                  HALF, HALF, W) 
               IF (DABS(W) < EPS) CYCLE  
               DO I4 = IP, IG 
                  KKL1 = I4 - 1 
...
! Spin-spin operator
                  ...
                  ENDIF 
                  IF (IAT == 0) CYCLE  
                  CALL NINELS (2*ID3(3), 2*ID1(3), 2*KL1, 2*ID3(3), 2*ID2(3), 2&
                     *KL2, 2*KKL2, 2*KKL1, 2*K, 1, INA, S) 
                  CALL NINELS (2*ID3(3), 2*ID2(3), 2*KL1, 2*ID3(3), 2*ID1(3), 2&
                     *KL2, 2*KKL2, 2*KKL1, 2*K, 1, INB, S) 
                  IF (INA + INB == 0) CYCLE  
                  IF (IRL(I4,I3) == 0) THEN 
                     IRL(I4,I3) = 1 
                  ...
                  IF (INA /= 0) THEN 
                     CALL NINELS (2*ID3(3), 2*ID1(3), 2*KL1, 2*ID3(3), 2*ID2(3)&
                        , 2*KL2, 2*KKL2, 2*KKL1, 2*K, 0, INA, SLA1) 
                     SLA1 = SLA1*DSQRT(DBLE((2*KKL1 + 1)*(2*KKL2 + 1))) 
                     IF (MOD(KKL1 + KKS1 + KKL2 + KKS2,2) /= 0) SLA1 = -SLA1 
                  ELSE 
                     SLA1 = ZERO 
                  ENDIF 
                  IF (INB /= 0) THEN 
                     CALL NINELS (2*ID3(3), 2*ID2(3), 2*KL1, 2*ID3(3), 2*ID1(3)&
                        , 2*KL2, 2*KKL2, 2*KKL1, 2*K, 0, INB, SLB1) 
                     SLB1 = SLB1*DSQRT(DBLE((2*KKL1 + 1)*(2*KKL2 + 1))) 
                     IF (MOD(ID1(3)+ID2(3)+KKL2+KKS2,2) /= 0) SLB1 = -SLB1 
                  ELSE 
                     SLB1 = ZERO 
                  ENDIF 
