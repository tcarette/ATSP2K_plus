...
!-----------------------------------------------
      INTEGER :: IAA, IBB, IAT, LIA2, LIB2, LA, LB, LC, LD, IP2, IKK, IG2, I2, &
         KL, I3, L12, I13, JS12, IFAZ, NN, IB1, II, KLL 
      REAL(DOUBLE) :: REC, RECS, RECL, QM1, QM2, A1, A2, AB, AC, AD, SI, AA, &
         ABB 
!-----------------------------------------------
      IF (IHSH <= 1) RETURN  
      IF (IA == IB) RETURN  
...
!
      ...
      DO I2 = IP2, IG2 
         ...
         IF (ICOLOM + ISOTOP == 1) CALL COULOMBLS (LJ(IB), LJ(IB), LJ(IB), LJ(&
            IA), KL, A1) 
         IF (IORBORB == 1) CALL ORBITORBIT (LJ(IB), LJ(IB), LJ(IB), LJ(IA), KL&
             + 1, A2) 
         IF (DABS(A1) + DABS(A2) <= EPS) CYCLE  
         AB = ZERO 
         DO I3 = IP2, IG2 
            L12 = I3 - 1 
            IF (IXJTIK(LIB2,LIA2,KL*2,LIB2,LIB2,L12*2) == 0) CYCLE  
            AC = ZERO 
            DO I13 = 1, 2 
               JS12 = I13 - 1 
               IFAZ = L12 + JS12 
               IF ((IFAZ/2)*2 /= IFAZ) CYCLE  
               CALL WA1A2LS (IK2, IK1, BK2, BK1, ID2, ID1, BD2, BD1, L12, JS12&
                  , QM1, QM1, QM2, QM2, AD) 
