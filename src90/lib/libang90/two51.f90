...
! Spin-spin operator
      ...
      IF (IWAA(1,1,KL1+1,KL2+1) == 0) THEN 
         ...
         END DO 
         IF (IREZ == 1) THEN 
            IF (MOD(ID3(3)+ID4(3),2) /= 0) CA1 = -CA1 
            IF (MOD(ID3(3)+ID4(3)+KL1+KL2,2) /= 0) CB1 = -CB1 
         ELSE 
            IF (MOD(ID1(3)+ID2(3)+KL1+KL2,2) /= 0) CA1 = -CA1 
            IF (MOD(ID1(3)+ID2(3),2) /= 0) CB1 = -CB1 
         ENDIF 
