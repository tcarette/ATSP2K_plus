...
*  B I O M A T
*
* Written by Michel Godefroid and Per Jonsson
*               Malmo, January 2008
*
*  Things to do:
*
...
**************************************************************************
*
*  This program will calculate the matrix element of one- and two-body operators between
*  two CSF expansions built on non-orthonormal orbitals (separately optimized). 
*  It should allow the calculation of
*
*  - overlap
*  - non-relativistic Hamiltonian
*  - relativistic Breit-Pauli operators
*
*
*  References: 
*  ----------
*  1. P.A. Malmqvist, Int.J. of Quantum Chemistry, XXX, 479-94 (1986)
*  2. J. Olsen, M.R. Godefroid, P. Jonsson, P.A. Malmqvist and 
*     C. Froese Fischer, Phys. Rev. E 52(1995)4499-4508
*
**************************************************************************
*
      PROGRAM BIOMAT
     ...
     : LFILE(2),LINTF(2),RWFILE(2),RLFILE(2)
     ...
     :  iuc(2),iuw(2),iul(2),iuj(2),iut(2),iurw(2),iurl(2)
...
*  iurw(1) - The unit number for the <name>.rw file for rotated initial state
*  iurw(2) - The unit number for the <name>.rw file for rotated final   state
*  iurl(1) - The unit number for the <name>.rl file for rotated initial state
*  iurl(2) - The unit number for the <name>.rl file for rotated final   state
...
*
      do 2 i = 1,2
        ...
        rwfile(i) = name(i)(1:j-1)//'.rw'
        rlfile(i) = name(i)(1:j-1)//'.rl'
...
*     orbitals
      WRITE(6,'(A)')
      WRITE(6,'(A)') ' ORIGINAL overlaps BETWEEN the two spaces '
      CALL BRKT
      WRITE(6,'(A)')
*
* --- check orthonormality of orbitals within each state
      WRITE(6,'(A)') ' ORIGINAL overlaps WITHIN each space '
      CALL BRKTIN(1)
      WRITE(6,'(A)')
      CALL BRKTIN(2)
      WRITE(6,'(A)')
...
* --- check biorthonormality
      WRITE(6,'(A)')
      WRITE(6,'(A)') ' Check biorthonormality after BIOTRN '
      CALL BRKT
      WRITE(6,'(A)')
*
* --- for our information, write the overlaps within each state
      WRITE(6,'(A)')
      WRITE(6,'(A)') ' Overlaps WITHIN each space, after BIOTRN '
      CALL BRKTIN(1)
      WRITE(6,'(A)')
      CALL BRKTIN(2)
      WRITE(6,'(A)')
*
* --- write the rotated orbital sets on files .rw
      call WRITERW
*
* --- write the counter-rotated CSF expansions on files .rl
      call WRITERL
*
 1000 Continue
*
