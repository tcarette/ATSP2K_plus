...
*
      CHARACTER YES
      DOUBLE PRECISION ME
      REAL MN(NELEMENT),FZ(NELEMENT-9)
*
...
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	if (myid.eq.0) then
      ...
      IF (IPROB.NE.1) THEN
       ...
       ENDIF
      ENDIF
      call MPI_BCAST(RYDBRG,1,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,
     $               ierr)
      call MPI_BCAST(ZMU,1,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,
     $               ierr)
      call MPI_BCAST(FZE,1,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
