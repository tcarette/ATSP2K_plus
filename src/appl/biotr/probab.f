...
*
      SUBROUTINE 
     :      PROBAB(ICI,IULS,IULSJ,NPAIR,CONFIGI,CONFIGF,IM,IPRINT)
...
CGG        ryrat = rydbrg/109737.31534
      ...
      if(ibugm.ne.0)print*,' k = ',k,' sl(k) = ',sl(k),' sv(k) = ',sv(k)
        if (da .eq. d0) go to 1
        d = dabs(da)
        dd = d*d2*rydbrg
        angs = d10**8/dd
...
*          da = -da
        ...
        if(ibugm.ne.0) write(iwrite,8) sl(k),trpt,aki
        if(ibugm.ne.0.and.vok) write(iwrite,9) sv(k),trpt,akiv
        fe = (angs**2)*1.499193d-16
        if (ibugm.ne.0) write(iwrite,10) i,fe,jvi,d1
...
*
      ...
      iMN = 0;
      do mo = 1, maxorb
         write(tmps3,'(A3)') IAJCMP(mo)
         itmp = ICHAR(tmps3(2:2)) - ICHAR('1') + 1
         if (iMN < itmp) iMN = itmp;
      end do
      MIZ = Z;
      write(iuls,'(/A5,I3,A5,I2)') ' Z = ', MIZ, ' n = ', iMN
!      WRITE(iuls,'(/)')
      if (da_tmp>0) then
