...
CG 
      ...
      if (myid == 0 )  then
         write(IWRITE,50)
         write(iscw,'(A/A)')
         write(iscw,*) '                 ...breit_mpi running on ',
     :                  nprocs,' processors...'
         write(iscw,*)
      end if
     ...
     :     IORBORB,ICOLOM,iwrite,IFULL,ICSTAS,skip)
      ...
      call MPI_BCAST(IFULL,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr);
      call MPI_BCAST(ICSTAS,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr);
...
*
      ...
      NBUG7 = 0
*
      if(myid == 0) WRITE(IWRITE,11) IREL,ICSTAS
*
...
*
      ...
      call MPI_BCAST(istrict,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr);
