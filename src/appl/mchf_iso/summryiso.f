...
*
Cmrg             1
Cmrg  Calculate Q            = grad(nl,n'(l-1) needed for yiso
Cmrg              nl,n'(l-1)

      open(unit=192,file='gradint',status='unknown')
      write(192,193) Etotal,nwf
      do 178 i = 1,nwf
         do 179 j = 1,nwf
            if(abs(l(i)-l(j)).eq.1) then
               write(192,194) el(i),el(j),grad(i,j)
            else
               write(192,194) el(i),el(j),0.d0
            endif
179      continue
178   continue
      close(192)
