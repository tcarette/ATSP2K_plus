...
!## read the name of the atom 
      subroutine inp_atom(QREL,QMASS,ATOMC,ATOML,ATOMJ,ATOMW,ATOMNEW,
     :      REL,MASS,iscw,iread)
...
!## select Gradient or Slater
      subroutine inp_gs(QREL,REL,MASS,QMASS,iread,iscw)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      PARAMETER (NOD=220, NWD=60)
      LOGICAL REL
      CHARACTER QREL, QMASS
...
!## convert y or n to true or false     
      subroutine inp_yn(yn,iscw,iread)
...
!## get the type of calculation: 0, 1 or 2 and S-O,SOO,SS,OO
      subroutine inp_type(irel,iread,iscw,ISPORB,ISOORB,ISPSPN,
     :                   IORBORB,iwrite)      
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      character ans*2

      integer iscw, iread, irel 
