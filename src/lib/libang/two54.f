...
* Spin-spin operator
	  ...
	  IF(K2KS3.EQ.3)KIIS1=2
	    ...
	    RW1(KIIS1,2)=RECS1
          ...
          IF(ISES1.NE.0) CALL SIXJ(2*KS1,2*KS2,2*K,K2KS3,1,1,0,SN1)
          IF(ISES2.NE.0) CALL SIXJ(2*KS2,2*KS1,2*K,K2KS3,1,1,0,SN2)
      DO 2 I1=IPS2,2
          KKS1=I1-1
          ISES3=IXJTIK(2*KKS1,1,1,2*KS2,1,K2KS3)
          ISES4=IXJTIK(2*KKS1,1,1,2*KS1,1,K2KS3)
    ...
    1   CONTINUE
	IF(IREZ.EQ.1) THEN
	  IF(MOD(KS1,2).NE.0) CAS1=-CAS1
	  IF(MOD(KS1,2).NE.0) CBS1=-CBS1
        ELSE
          IF(MOD(KS2,2).NE.0) CAS1=-CAS1
          IF(MOD(KS2,2).NE.0) CBS1=-CBS1
        ENDIF
      ...
      IF(IWAA(2,1,KL1+1,KL2+1).EQ.0) THEN
        ...
        DO 3 I3=IP1,IG1
          ...
          IG2=IP2+IKK2-1
          ISES1L=IXJTIK(2*KL1,2*KL2,2*K,2*KKL3,2*ID4(3),2*ID1(3))
          ISES2L=IXJTIK(2*KL2,2*KL1,2*K,2*KKL3,2*ID4(3),2*ID1(3))
	    ...
	    RW2(1,I3)=RECL1
          ...
          IF(ISES1L.NE.0) 
     :       CALL SIXJ(2*KL1,2*KL2,2*K,2*KKL3,2*ID4(3),2*ID1(3),0,S3)
	  IF(ISES2L.NE.0) 
     :       CALL SIXJ(2*KL2,2*KL1,2*K,2*KKL3,2*ID4(3),2*ID1(3),0,S4)
        DO 4 I4=IP2,IG2
          KKL1=I4-1
          ISES3L=IXJTIK(2*KL2,2*ID3(3),2*ID2(3),2*KKL1,2*ID1(3),2*KKL3)
          ISES4L=IXJTIK(2*KL1,2*ID3(3),2*ID2(3),2*KKL1,2*ID1(3),2*KKL3)
     ...
     :              (2,IA,IB,IC,2*ID1(3),2*ID2(3),2*KKL1,1,IAT,RECL2)
	        ...
	        IF(ISES1L*ISES3L.NE.0) THEN
                  CALL SIXJ
     :          (2*KL2,2*ID3(3),2*ID2(3),2*KKL1,2*ID1(3),2*KKL3,0,SLA1)
                  SLA1=SLA1*DSQRT(DBLE((2*KKL1+1)*(2*KKL3+1)))
                  CA1=CA1+S3*SLA1*RECL1*RECL2*RECL3
                ENDIF
	        IF(ISES2L*ISES4L.NE.0) THEN
                  CALL SIXJ
     :          (2*KL1,2*ID3(3),2*ID2(3),2*KKL1,2*ID1(3),2*KKL3,0,SLB1)
    ...
    3   CONTINUE
	IF(IREZ.EQ.1) THEN
	  IF(MOD(ID1(3)+ID4(3)+KL1,2).NE.0) CA1=-CA1
          IF(MOD(ID1(3)+ID4(3)+KL1,2).NE.0) CB1=-CB1
        ELSE
	  IF(MOD(ID2(3)+ID3(3)+KL2,2).NE.0) CA1=-CA1
	  IF(MOD(ID2(3)+ID3(3)+KL2,2).NE.0) CB1=-CB1
        ENDIF
