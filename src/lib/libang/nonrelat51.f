...
*
     ...
     *ID3(7),ID4(7),IK3(7),IK4(7)
     ...
     :BD1,BD2,BD3,BD4,QM1,QM1,QM2,QM2,ANG)
...
C
C     CASES 1234   + + - -
C           2134                  TRANSFORM TO  1234   + + - -
C                                                    (IREZ = 1)
C     OR
C     CASES 3412   + + - -        TRANSFORM TO  1234   - - + +
C           3421                                1234
C                                                    (IREZ = 2)
	...
	IF(CALCULATION(KL)) THEN
     ...
     :           CALL COULOMBLS(LJ(IA),LJ(IB),LJ(IC),LJ(ID),KL,A1)
          IF(IORBORB.EQ.1) 
     :           CALL ORBITORBIT(LJ(IA),LJ(IB),LJ(IC),LJ(ID),KL+1,A2)
    ...
    4             CONTINUE
	          ...
	          IF(IREZ.EQ.2) IFAZ=IK1(3)+IK4(3)+KL+L12
    ...
    3       CONTINUE
     ...
     :               CALL SAVENON(3,AB,KL,LA,LB,LC,LD,JA,JB,0)
	          IF(IREZ.EQ.2)
     :               CALL SAVENON(3,AB,KL,LC,LD,LA,LB,JA,JB,0)
  	        ENDIF
  	      ENDIF
C Orbit 1234
...
C                 WRITE(79,656) ABB,KLL,LJ(IA),LJ(IB),JA,JB
  656 FORMAT(1X,'1221 O-O','AA=',F17.7,'K=',I3,'LA=',2I3,'JA=',I4,
    ...
    2 CONTINUE
C
C     CASES 1243   + + - -        TRANSFORM TO  1234   + + - -
C           2134                                1234
C                                                    (IREZ = 1)
C     OR
C     CASES 3421   + + - -        TRANSFORM TO  1234   - - + +
C           4321                                1234
C                                                    (IREZ = 2)
C
	...
	IF(CALCULATION(KL)) THEN
          IF((ICOLOM+ISOTOP).EQ.1) 
     :           CALL COULOMBLS(LJ(IA),LJ(IB),LJ(ID),LJ(IC),KL,A1)
          IF(IORBORB.EQ.1) 
     :           CALL ORBITORBIT(LJ(IA),LJ(IB),LJ(ID),LJ(IC),KL+1,A2)
          IF((DABS(A1)+DABS(A2)).GT.EPS) THEN
            AB=ZERO
            DO 13 I3=IP1,IG1
              L12=I3-1
              IF(IATL(I3).NE.0) THEN
                IF(IXJTIK(LIA2,LID2,KL*2,LIC2,LIB2,L12*2).NE.0) THEN
                  AC=ZERO
                  DO 14 I13=1,2
		    IF(IATS(I13).NE.0) THEN
                      JS12=I13-1
	              ...
	              IF(IREZ.EQ.2) IFAZ=IK1(3)+IK4(3)+KL+JS12
                      IF((IFAZ/2)*2.NE.IFAZ)AA=-AA
                      AC=AC+AA
	            ENDIF
   14             CONTINUE
   ...
   13       CONTINUE
     ...
     :                       CALL SAVENON(3,AB,KL,LA,LB,LD,LC,JA,JB,0)
                  IF(IREZ.EQ.2)
     :                       CALL SAVENON(3,AB,KL,LC,LD,LB,LA,JA,JB,0)
 	        ENDIF
 	      ENDIF
C Orbit 1243
              IF(IORBORB.EQ.1) THEN
