...
* Spin-spin operator
	...
	CB1=CB1/DSQRT(DBLE(2*KL2+1))
        IF(IREZ.EQ.1) THEN
          IF(MOD(ID3(3)+ID4(3)+KL1+1,2).NE.0) CA1=-CA1
          IF(MOD(ID3(3)+ID4(3)+KL1+1,2).NE.0) CB1=-CB1
        ELSE
          IF(MOD(ID1(3)+ID2(3)+KL2+1,2).NE.0) CA1=-CA1
          IF(MOD(ID1(3)+ID2(3)+KL2+1,2).NE.0) CB1=-CB1
        ENDIF
