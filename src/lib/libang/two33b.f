...
*
      SUBROUTINE TWO33B(KL1,KS1,KL2,KS2,K,IA,IB,IC,INE1,INE2,CA1,CB1)
...
* Spin-spin operator
      ...
      DO 3 I2=IPS2,2
        ...
        IF(IRS(I1,I2).EQ.0) THEN
          IRS(I1,I2)=1
          CALL RLSP3(3,IB,IA,IC,1,1,2*KKS1,2*KKS2,2*K,RECS)
	  RS(I1,I2)=RECS
        ELSE
	  RECS=RS(I1,I2)
        ENDIF
      ...
      DO 4 I3=IP1,IG1
        KKL2=I3-1
        CALL A1A2W3LSP(IB,IA,IC,IC,KKL2,KKS2,-HALF,HALF,HALF,-HALF,W)
        IF(DABS(W).LT.EPS) GO TO 4 
      DO 5 I4=IP,IG
        KKL1=I4-1
        CALL NINELS(2*ID1(3),2*ID3(3),2*KL1,2*ID2(3),2*ID3(3),2*KL2,
     :         2*KKL1,2*KKL2,2*K,1,INA,S)
        CALL NINELS(2*ID2(3),2*ID3(3),2*KL1,2*ID1(3),2*ID3(3),2*KL2,
     :         2*KKL1,2*KKL2,2*K,1,INB,S)
        IF((INA+INB).EQ.0) GO TO 5
        IF(IRL(I4,I3).EQ.0) THEN
          IRL(I4,I3)=1
          CALL 
     :     RLSP3(2,IB,IA,IC,2*ID2(3),2*ID1(3),2*KKL1,2*KKL2,2*K,RECL)
	  RL(I4,I3)=RECL
        ELSE
	  RECL=RL(I4,I3)
        ENDIF
        ...
        IF(INA.NE.0) THEN
          CALL NINELS(2*ID1(3),2*ID3(3),2*KL1,2*ID2(3),2*ID3(3),2*KL2,
     :       2*KKL1,2*KKL2,2*K,0,INA,SLA1)
          SLA1=SLA1*DSQRT(DBLE((2*KKL1+1)*(2*KKL2+1)))
          IF(MOD(KKL1+KKS1+KL1+KS1+ID2(3)+ID3(3),2).NE.0) SLA1=-SLA1
        ELSE
          SLA1=ZERO
        ENDIF
        IF(INB.NE.0) THEN
          CALL NINELS(2*ID2(3),2*ID3(3),2*KL1,2*ID1(3),2*ID3(3),2*KL2,
     :         2*KKL1,2*KKL2,2*K,0,INB,SLB1)
          SLB1=SLB1*DSQRT(DBLE((2*KKL1+1)*(2*KKL2+1)))
          IF(MOD(ID1(3)+ID3(3)-KL2-KS2-KKL2-KKS2,2).NE.0) SLB1=-SLB1
        ELSE
          SLB1=ZERO
        ENDIF
