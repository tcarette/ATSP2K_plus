...
*
     ...
     *ID3(7),ID4(7),IK3(7),IK4(7)
     ...
     :BD1,BD2,BD3,BD4,QM1,QM2,QM1,QM2,ANG)
    ...
    8 CONTINUE
      IF((NN/2)*2.EQ.NN)IFAZP=-IFAZP
C
C     CASES 1324   + + - -        TRANSFORM TO  1234   + - + -
C           1342                                1234
C                                                    (IREZ = 1)
C     OR
C     CASES 2413   + + - -        TRANSFORM TO  1234   - + - +
C           4231                                1234
C                                                    (IREZ = 2)
      IF(IATS(1).NE.0) THEN
        DO 2 I2=IP1,IG1
     ...
     :	                              /DSQRT(DBLE(2*KL+1))
                  IFAZ=2
	          IF(IREZ.EQ.2) IFAZ=IK1(3)+IK4(3)+IK2(3)+IK3(3)
                  IF((IFAZ/2)*2.NE.IFAZ)AB=-AB
                    IF(DABS(AB).GT.EPS) THEN
                    IF(IREZ.EQ.1)
     :                    CALL SAVENON(3,AB,KL,LA,LC,LB,LD,JA,JB,0)
                    IF(IREZ.EQ.2)
     :                    CALL SAVENON(3,AB,KL,LB,LD,LA,LC,JA,JB,0)
 	          ENDIF
	        ENDIF
	      ENDIF
	    ENDIF
...
C     :                         WRITE(79,556) AB,KL,LJ(IA),LJ(IB),JA,JB
  556 FORMAT(1X,'1221 O-O','AA=',F17.7,'K=',I3,'LA=',2I3,'JA=',I4,
    ...
    2   CONTINUE
      ENDIF
C
C     CASES 1342   + + - -        TRANSFORM TO  1234   + - + -
C           3124                                1234
C                                                    (IREZ = 1)
C     OR
C     CASES 2431   + + - -        TRANSFORM TO  1234   - + - +
C           4213                                1234
C                                                    (IREZ = 2)
     ...
     :           CALL COULOMBLS(LJ(IA),LJ(IC),LJ(ID),LJ(IB),KL,A1)
          IF(IORBORB.EQ.1) 
     :           CALL ORBITORBIT(LJ(IA),LJ(IC),LJ(ID),LJ(IB),KL+1,A2)
	      ...
	      ABB=AB
              IF((ICOLOM+ISOTOP).EQ.1) THEN
                IF(DABS(A1).GT.EPS) THEN
                  AB=A1*AB
                  IF(IREZ.EQ.1)
     :                 CALL SAVENON(3,AB,KL,LA,LC,LD,LB,JA,JB,0)
                  IF(IREZ.EQ.2)
     :                 CALL SAVENON(3,AB,KL,LB,LD,LC,LA,JA,JB,0)
 	        ENDIF
 	      ENDIF
C Orbit 1342
              IF(IORBORB.EQ.1) THEN
