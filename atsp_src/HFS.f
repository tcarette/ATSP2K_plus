...
*     max NCD=2000 configurations (change the parameter NCD
*     in this routine and in the subroutines in this program unit)

      PROGRAM HFS 

      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      PARAMETER (NOD=220,NWD=30,NCD=2000,NORD=NWD*(NWD-1)/2)
...
*
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      PARAMETER(NOD=220,NWD=30,NCD=2000)
      INTEGER SMAX(NWD)
...
*     Tensorial Sets.
      ...
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      PARAMETER(NOD=220,NWD=30,NCD=2000,EPSILO=1.D-9)
      COMMON/TENSOR/VVSHELL(50),IIHSH,NNOVLP(50),IIRHO(50),IISIG(50),
     : MMU(50),MMUP(50),NNU(50),NNUP(50)
      COMMON/HYPER/NHY,MHY,IRHY(20),ISHY(20),JRHY(20),JSHY(20),VHY(20),
     1 IMUHY(20),IMUPHY(20),JMUHY(20),JMUPHY(20),
...
820   FORMAT(15H matrix element,
     ...
     :2X,69HJ    J'   weight    coeff       matrix element values               
     :   B(MHz))
830   FORMAT( A4,1X,A4,1X,F9.6,1X,F9.6,1X,F9.4,17X,F12.6,F12.6)
840   FORMAT( A4,1X,A4,1X,F9.6,1X,F9.6,1X,F9.4,1X,F7.3,91X,
     :F12.6,F12.6)
850   FORMAT( A4,1X,A4,1X,F9.6,1X,F9.6,1X,F9.4,1X,F7.3,11X,F7.3,11X,
     :F12.6,F12.6)
860   FORMAT( A4,1X,A4,12X,F10.7)
...
*
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      PARAMETER(KFL1=60,KFL2=12)
      PARAMETER (NWD=30,NCD=2000,NORD=NWD*(NWD-1)/2)
*
